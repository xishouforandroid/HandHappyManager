...
!function (a, b, c, d) {
    var e = function (a, b) {
        ...
        return d.prototype.build = function () {
            ...
            return this.items.filter("." + d["arrow" + c.Helper.capitalise(a)]).unbind("click.glide touchstart.glide").addClass(d.disabled).siblings().bind("click.glide touchstart.glide", this.click).removeClass(d.disabled)
        ...
        return d.prototype.init = function () {
            ...
            return this.items.eq(b.current - 1).addClass("active").siblings().removeClass("active")
        ...
        return c.prototype.init = function () {
            ...
            var b, c;
            for (c = 0; c < this.items.length; c++)b = this.items[c][a.size](a[a.size]), d[c] >= 0 ? b.appendTo(a.track) : b.prependTo(a.track)
        ...
        return e.prototype.keyboard = function () {
            ...
            a(b).on("resize.glide", d.Helper.throttle(function () {
            ...
            return c.track.off("click", "a"), this
        }, e.prototype.preventClicks = function (a) {
            return c.track.one("click", "a", function (a) {
                a.preventDefault()
            }), this
            ...
            c.track.off("keyup.glide").off("mouseover.glide").off("mouseout.glide"), f.off("click.glide touchstart.glide"), a(b).off("keyup.glide").off("resize.glide")
        }, new e
    }, m = function (a, b) {
        function c() {
        ...
        return c.prototype.play = function () {
            ...
            return a.slides.eq(a.current - 1).data("glide-autoplay") || a.options.autoplay
        ...
        return d.prototype.unbind = function () {
            ...
            if (!c.Events.disabled && this.dragging) {
                ...
                var d = parseInt(e.pageX) - this.touchStartX, f = parseInt(e.pageY) - this.touchStartY, g = Math.abs(d << 2), h = Math.abs(f << 2), i = Math.sqrt(g + h), j = Math.sqrt(c.Helper.byAxis(h, g));
                if (this.touchSin = Math.asin(j / i), this.distance = c.Helper.byAxis(e.pageX - this.touchStartX, e.pageY - this.touchStartY), c.Animation.make(c.Helper.byAxis(d, f)), c.Events.preventClicks().call(b.options.swipeMove).trigger("swipeMove"), c.Build.isMode("vertical")) {
            ...
            if (!c.Events.disabled && this.dragging) {
                ...
                var d = c.Helper.byAxis(e.pageX - this.touchStartX, e.pageY - this.touchStartY), f = 180 * this.touchSin / Math.PI;
                c.Transition.jumping = !1, c.Build.isType("slider") && (c.Run.isStart() && d > 0 && (d = 0), c.Run.isEnd() && 0 > d && (d = 0)), d > b.options.touchDistance && 45 > f ? c.Run.make("<") : d < -b.options.touchDistance && 45 > f ? c.Run.make(">") : c.Animation.make(), c.Animation.after(function () {
                    c.Events.enable(), c.Run.play()
                }), this.dragging = !1, c.Events.disable().call(b.options.swipeEnd).trigger("swipeEnd"), b.track.removeClass(b.options.classes.dragging).off("touchmove.glide mousemove.glide").off("touchend.glide touchcancel.glide mouseup.glide mouseleave.glide")
            }
        }, new d
    }, q = function (a, b) {
        function c() {
        ...
        this.options = a.extend({}, d, c), this.current = parseInt(this.options.startAt), this.element = b, this.collect(), this.setup(), this.options.beforeInit({
            index: this.current,
