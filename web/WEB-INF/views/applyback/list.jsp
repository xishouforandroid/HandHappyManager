...
<div class="row">
    <div id="breadcrumb" class="col-xs-12">
        ...
        <ol class="breadcrumb pull-left">
            ...
            <li><a href="javascript:void(0)">申请列表</a></li>
        </ol>
            ...
            <div class="box-content">
                ...
                <table class="table table-hover">
                    ...
                    <c:forEach items="${list}" var="e" varStatus="st">
                        <tr>
                            <td>${e.nicknameJbr}</td>
                            <td>${um:format(e.applytime, 'yyyy-MM-dd')}</td>
                            <td>${um:format(e.donetime, 'yyyy-MM-dd')}</td>
                            <td>
                                <c:if test="${e.is_check=='0'}">否</c:if>
                                <c:if test="${e.is_check=='1'}">是</c:if>
                            </td>
                            <td>
                                <a class="btn btn-default btn-sm" href="javaScript:void(0)" onclick="toCheck('${e.applyid}','${e.is_check}')"
                                   role="button">处理</a>
                            </td>
                        </tr>
                    </c:forEach>
...
<script type="text/javascript">
    function searchIndex(e) {
        ...
        if (_index <= ${page.pageCount} && _index >= 1) {
            window.location.href = "#module=/applyBack/list&page=" + _index
            + "&size=" + size
            + "&_t=" + new Date().getTime();
        } else {
    ...
    function nextPage(_page) {
        ...
        if ((page <= ${page.pageCount} && page >= 1)) {
            window.location.href = "#module=/applyBack/list&page=" + page
            + "&size=" + size
            + "&_t=" + new Date().getTime();
        } else {
            alert("请输入1-${page.pageCount}的页码数");
    ...
    function toCheck(_applyid, _ischekc) {
        if(_ischekc == '1'){
            alert("不能重复处理！");
        }else{
            window.location.href = "#module=/applyBack/toEdit&applyid=" + _applyid
        }
    }
