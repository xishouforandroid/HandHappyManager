                ...
                <table class="table table-hover">
                    ...
                    <c:forEach items="${list}" var="e" varStatus="st">
                        <tr>
                            ...
                            <td>
                                <c:if test="${e.is_use=='0'}">启用</c:if>
                                <c:if test="${e.is_use=='1'}">禁用</c:if>
                            </td>
                            <td>
                                <a class="btn btn-default btn-sm" href="#module=/admin/detail&manager_id=${e.manager_id}"
                                   role="button">管理</a>
                            </td>
                        </tr>
                    </c:forEach>
...
<script type="text/javascript">
    function searchIndex(e) {
        ...
        if (_index <= ${page.pageCount} && _index >= 1) {
            window.location.href = "#module=/admin/list&page=" + page + "&size=" + size + "&is_use=" + is_use + "&_t=" + new Date().getTime();
        } else {
